...
def _get_vectorstore_docs() -> List[tuple[str, Document]]:
    if vectorstore is None or not hasattr(vectorstore, "docstore"):
        return []
...
def _doc_key(doc: Document) -> str:
    docstore_dict = getattr(vectorstore.docstore, "_dict", {}) if vectorstore is not None else {}
    for doc_id, stored_doc in docstore_dict.items():
        if stored_doc is doc:
            return doc_id
...
def build_bm25_index():
    global bm25_index, bm25_docs, bm25_doc_ids

    doc_items = _get_vectorstore_docs()
    bm25_doc_ids = [doc_id for doc_id, _ in doc_items]
    bm25_docs = [doc for _, doc in doc_items]
    tokenized_corpus = [tokenize_text(doc.page_content) for doc in bm25_docs]

    if not bm25_docs or not any(tokenized_corpus):
        bm25_index = None
        print("BM25 không có dữ liệu để khởi tạo")
        return

    bm25_index = BM25Okapi(tokenized_corpus)
    print(f"Đã khởi tạo BM25 với {len(bm25_docs)} documents")
...
def bm25_search(query: str, top_k: int = DEFAULT_SPARSE_TOP_K) -> List[Document]:
    if bm25_index is None:
        print("BM25 chưa được khởi tạo, bỏ qua sparse search")
        return []
...
def hybrid_search(query: str, top_k: int = DEFAULT_FINAL_TOP_K) -> List[Document]:
    global vectorstore
    if vectorstore is None:
        init_vector_db()

    dense_docs = vectorstore.similarity_search(query, k=DEFAULT_DENSE_TOP_K)
    sparse_docs = bm25_search(query, top_k=DEFAULT_SPARSE_TOP_K)
