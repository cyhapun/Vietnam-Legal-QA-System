...
The system MUST support legal retrieval through the new persistent storage layer utilizing Qdrant's native Hybrid Search (RRF) mechanism across both dense and sparse vectors, supporting multi-query execution. The async orchestration MUST dispatch each retrieval operation exactly once before pooling, deduplicating, and reranking its results.
